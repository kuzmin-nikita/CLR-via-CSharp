...
Несмотря на то, что для типов не существует статических методов `Finalize()`, выгрузить тип из домена можно, зарегистрироваа колбэк метод для события `DomainUnload` типа `System.AppDomain`.
