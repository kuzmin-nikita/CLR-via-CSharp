...
- К больишм объектам не применяется сжатие, так как на их перемещение требуется слишком много времени.
...
- **Режим сервера.** Оптимизирует сборку мусора на стороне сервера. Все ресурсы процессора тратятся на сборку мусора. Управляемая куча разибрается на несколько разделов - по одному на процессор. При работе с несколькими процессорами возможно распараллеливание сборки.
...
Помимо двух основных режимов существует два подрежима: параллельный (по умолчанию) и непараллельный. В параллельном режиме есть дополнительный фоновый поток, выполняющий разметку объектов во время работы приложения. Когда поток размещает в памяти объект, вызывающий превыщение порога поколения 0, сборщик сначала приостанавливает все потокуи, а затем определяет поколения, подлежащие очистке. Если необходимо очистить поколения 0 и 1, сборка мусора происходит как обычно. Если нужно очистить поколение 2, то размер поколения 0 увеличивается выше порогового и исполнение потоков продолжается.

Кроме того, в параллельной сборке память не дефрагментируется, что повышает рабочий набор приложения, но одновремменно с этим повышается и быстродействие.
...
Некоторым типам помимо памяти требуются ещё и системаные ресурсы (файлы, мьютексы и так далее).

Если тип, использующий ситсемный ресурс, будет уничтожен в ходе сборки мусора, занимаемая им память вернётся в управляемую кучу, но системный ресурс будет потерян. Это нежелеательно, поэтому CLR поддерживает механизм _финализации_ (finalization), позволяющий объекту выполнить корректную очистку перед тем, как сборщик мусора освободит занимаемую им память. Любой тип, использующий системный ресурс, должен поддерживать финализацию. Когда CLR определит объект как недоступный, ему предоставляяется возможность выполнить финализацию с освобождением всех задействованныз ресурсов, после чего объект будет возвращён в управляемую кучу.
...
Данные методы вызываются при завершении сблрки мусора для объектов, подлежащих удалению. Это означет, что память таких объектов не может быть освобождена немеленно, так как метод `Finalize()` может выполнить код с обращением к полю типа. Так как финализируемый объект должен пережить сборку мусора, он переводится в другое поколение, в следствие чего этот оъект живёт дольше. Ситуация не идеальна в отношении памяти, поэтому финализации стоит избегать. 
...
При создании управляемого типа, использующего системный ресурс, стоит наследоваться от специального базового класса [`System.Runtime.InteropServices.SafeHandle`](https://learn.microsoft.com/ru-ru/dotnet/api/system.runtime.interopservices.safehandle?view=net-8.0). Такие типы гарантируют освобождение системеного ресурса в ходе сборки мусора.
...
В классе, являющимся обёрткой системного ресурса с количественным ограничением, следует использовть экземпляр этого класса для передачи сборщику мусора информации о том, сколько реально задействовано экземпляров этого ресурса.
